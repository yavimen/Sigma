...
namespace PolynomialTask.Classes
{
    class Polynomial
    {
        ...
        {
            if (Array.Find(Monomials, v => v.Power == m.Power) == null)
            {
                Monomial[] temp = new Monomial[monomials.Length + 1];
                for (int i = 0; i < monomials.Length; i++)
                {
            ...
            {
                this.Monomials[Array.FindIndex(Monomials, v => v.Power == m.Power)].Coefficient += m.Coefficient;
            }
        ...
        public static Polynomial operator +(Polynomial p1, Polynomial p2) => p1.Add(p2);

        public static Polynomial operator -(Polynomial p1, Polynomial p2) => p1.Subtract(p2);

        public static Polynomial operator *(Polynomial p1, Polynomial p2) 
        {
            Polynomial ptemp = new Polynomial();

            for (int i = 0; i < p1.Monomials.Length; i++)
            {
                for (int j = 0; j < p2.Monomials.Length; j++)
                {
                    ptemp.AddMon(p1.Monomials[i] * p2.Monomials[j]);
                }
            }
        ...
        public static implicit operator Polynomial(double var)
        {
